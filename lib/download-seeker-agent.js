...
async function run() {
    try {
        ...
        const seekerAgentToken = utils_1.getInputOrEnvironmentVariable('seekerAgentToken', 'SEEKER_ACCESS_KEY', false // not required
        );
        const seekerAgentName = utils_1.getInputOrEnvironmentVariable('seekerAgentName', 'SEEKER_AGENT_NAME', false // not required
        );
        ...
        try {
            ...
            const url = `${seekerServerURL}/rest/api/latest/installers/agents/scripts/${technology}?projectKey=${seekerProjectKey}&downloadWith=curl&osFamily=${osFamily}&flavor=DEFAULT&agentName=${seekerAgentName}&accessToken=${seekerAgentToken}`;
