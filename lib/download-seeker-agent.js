...
async function run() {
    try {
        ...
        const seekerAgentToken = utils_1.getInputOrEnvironmentVariable('seekerAgentToken', 'SEEKER_ACCESS_TOKEN', false // not required
        );
        const technology = core.getInput('technology', { required: true });
        const osFamily = core.getInput('osFamily', { required: true });
        const webServer = core.getInput('webServer', { required: false });
        const agentName = core.getInput('agentName', { required: false });
        try {
            // First download the installer script from the Seeker server
            const url = `${seekerServerURL}/rest/api/latest/installers/agents/scripts/${technology}?projectKey=${seekerProjectKey}&downloadWith=curl&osFamily=${osFamily}&webServer=${webServer}&flavor=DEFAULT&agentName=${agentName}&accessToken=${seekerAgentToken}`;
